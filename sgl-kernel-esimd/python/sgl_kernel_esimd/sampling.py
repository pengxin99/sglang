...
import torch
from sgl_kernel_esimd.utils import _to_tensor_scalar_tuple, get_cuda_stream
...
def _top_k_renorm_probs_internal(
    ...
    torch.ops.sgl_kernel_esimd.top_k_renorm_probs.default(
...
def _top_p_renorm_probs_internal(
    ...
    renorm_probs = torch.empty_like(probs)
    torch.ops.sgl_kernel_esimd.top_p_renorm_probs.default(
...
def _top_p_sampling_from_probs_internal(
    ...
    with probs.device as device:
        ...
        torch.ops.sgl_kernel_esimd.top_p_sampling_from_probs.default(
            probs,
            samples,
            indices,
            maybe_top_p_arr,
...
def _top_k_top_p_sampling_from_probs_internal(
    ...
    with probs.device as device:
        ...
        samples = torch.empty(probs.size(0), dtype=torch.int32, device=device)
        torch.ops.sgl_kernel_esimd.top_k_top_p_sampling_from_probs.default(
            probs,
            samples,
            indices,
            maybe_top_k_arr,
...
def _min_p_sampling_from_probs_internal(
    ...
    with probs.device as device:
        ...
        samples = torch.empty(probs.size(0), dtype=torch.int32, device=device)
        torch.ops.sgl_kernel_esimd.min_p_sampling_from_probs.default(
            probs,
            samples,
            indices,
            maybe_min_p_arr,
