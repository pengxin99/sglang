...
def lightning_attention_decode(q, k, v, past_kv, slope, output, new_kv):
    torch.ops.sgl_kernel_esimd.lightning_attention_decode.default(
...
def merge_state(
    ...
    torch.ops.sgl_kernel_esimd.merge_state.default(v_a, s_a, v_b, s_b, v_merged, s_merged)
...
def merge_state_v2(
    ...
    if s_merged is None:
        s_merged = torch.empty_like(s_a)
    torch.ops.sgl_kernel_esimd.merge_state_v2.default(v_a, s_a, v_b, s_b, v_merged, s_merged)
    return v_merged, s_merged
...
def cutlass_mla_decode(
    ...
    torch.ops.sgl_kernel_esimd.cutlass_mla_decode.default(
...
def cutlass_mla_get_workspace_size(
    ...
    return torch.ops.sgl_kernel_esimd.cutlass_mla_get_workspace_size.default(
