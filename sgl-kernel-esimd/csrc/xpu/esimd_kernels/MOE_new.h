...
static void moe_forward_up_kernel_32g(
    ...
    auto cgf = [&](sycl::handler& handle) {
        handle.parallel_for(
            ...
            [=](sycl::nd_item<2> item) SYCL_ESIMD_KERNEL {
                ...
                // const uint8_t* scales1 = weight1 + (int64_t)output_size * nb * BLOCK_SIZE;
                // const uint8_t* scales2 = weight2 + (int64_t)output_size * nb * BLOCK_SIZE;
                ...
                for (int s = tid; s < nsb; s += GS) {
                    ...
                    #pragma unroll
                    for (int v = 0; v < VS; ++v) {
                        simd<fp16, SBS * QK> yvs1 = load_qblocks(
                        ...
                        simd<fp16, SBS * QK> yvs2 = load_qblocks(
...
template <typename IT, const int VS, const int GS, const int ES>
static void moe_forward_down_kernel_32g(
    const void* input_ptr,
    const int64_t* indexs,
    const void* eweights,
        ...
        handle.parallel_for(
            ...
            [=](sycl::nd_item<2> item) SYCL_ESIMD_KERNEL {
                ...
                for (int s = tid; s < nsb; s += GS) {
                    ...
                    #pragma unroll
                    for (int v = 0; v < VS; ++v) {
                        simd<fp16, SBS * QK> yvs = load_qblocks(
