                  ...
                  simd<uint16_t, HD> result{0.0};
                  {
                    ...
                    // subnormal input
                    {
                      ...
                      simd<uint16_t, HD> vec = mantissa;
                      {
                        ...
                        // 4. 计算前导零: 158 - exponent
                        simd<uint32_t, HD> lz = 158 - exponent_tmp; // 158 = 127(指数偏移) + 31(32位整数)
...
/*
                  ...
                  {
                    // simd<uint16_t, HD> h{0.0};
                    // simd<uint16_t, HD> sign{0.0};
                    simd<uint16_t, HD> result{0.0};
                    
                    #pragma unroll
                    for (int i = 0; i < HD; i++)
                    {
                      uint8_t x = weight_quanted[i];
                      constexpr uint16_t weo = 5;
                      constexpr uint16_t wmo = 10;
                      ...
                      uint16_t mantissa = x & ((1 << WM) - 1);
                      uint16_t exponent = (x & 0x7F) >> WM;
                    
                      // subnormal input
                      if (exponent == 0) {
                        // guaranteed mantissa!=0 since cases 0x0 and 0x80 are handled above
                        // uint16_t renorm_shift = __builtin_clz(mantissa);

                        // uint16_t renorm_shift;
                        // {
                        //   uint16_t x = mantissa;
                        //   if (x == 0)
                        //   {
                        ...
                        simd<uint16_t, 1> renorm_shift_vec;
                        simd<uint16_t, 1> vec = mantissa;
                        {
                          // 1. 检查0值 (CLZ(0)=16)
                          auto is_zero = (vec == 0);
                          
                          // 2. 16位整数转32位浮点 (ESIMD自动零扩展)
                          simd<float, 1> vec_float = vec;
                          
                          // 3. 提取浮点数指数域 (位31:23)
                          simd<uint32_t, 1> vec_uint = vec_float.template bit_cast_view<uint32_t>();
                          simd<uint32_t, 1> exponent = (vec_uint >> 23) & 0xFF;
                          
                          // 4. 计算前导零: 158 - exponent
                          simd<uint32_t, 1> lz = 158 - exponent; // 158 = 127(指数偏移) + 31(32位整数)
                        ...
                        uint16_t renorm_shift = renorm_shift_vec[0];

                        // renorm_shift111 = (IT)renorm_shift;

                        uint16_t sh = 1 + renorm_shift - (32 - WM);
                        mantissa <<= sh;
                        exponent += 1 - sh;
                        mantissa &= ((1 << WM) - 1);
                      }
                      ...
                      uint16_t sign = x >> 7;
                      uint16_t retval = (sign << 15) | (exponent << 10) | mantissa;
